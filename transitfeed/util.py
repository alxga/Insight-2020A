...
def DecodeUnicode(text, errorsHandling="strict"):
  """
  Optionally decode bytes and return it as text. The result should be safe
  to print.
  """
  if isinstance(text, bytes):
    return text.decode(OUTPUT_ENCODING, errorsHandling)
  else:
    return text
...
def IsValidHexColor(color):
  ...
  """
  return re.match("^[0-9a-fA-F]{6}$", color) is not None
...
def DateStringToDateObject(date_string):
  ...
  if re.match(r"^\d{8}$", date_string) is None:
    return None
  try:
